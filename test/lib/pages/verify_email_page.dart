...
class _VerifyEmailPageState extends State<VerifyEmailPage> {
  ...
  Future<void> _sendAgain() async {
    ...
    final res = await MockApiService.instance.sendVerification(email);
    setState(() => loading = false);
    _showSnack(res.body['message'] ?? '发送完成（模拟）');
  }
  ...
  Future<void> _verify() async {
    ...
    setState(() => loading = true);
    final res = await MockApiService.instance.verifyCode(email.trim(), code.trim());
    setState(() => loading = false);
    if (res.statusCode == 200) {
      _showSnack('验证成功，注册完成！请登录');
